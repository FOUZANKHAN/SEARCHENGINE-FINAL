...
class BooleanQueryParser:
    ...
    def _find_next_literal(subquery : str, start_index : int) -> 'BooleanQueryParser._Literal':
        ...
        if subquery[start_index] == '-':
            #  is_negative_component = True    # Use recursion to extract the next literal following the - and wrap it inside a NotQuery component
            ...
            return BooleanQueryParser._Literal(
                ...
                NotQuery(not_query_literal.literal_component, is_not=True))
        ...
        if subquery[start_index] == '[':
            ...
            return BooleanQueryParser._Literal(
                ...
                NearLiteral(subquery[start_index:start_index + length_out],is_not = False))
        ...
        if subquery[start_index] == '"':
            ...
            return BooleanQueryParser._Literal(
            BooleanQueryParser._StringBounds(start_index, length_out+1),
            PhraseLiteral(subquery[start_index:start_index + length_out],is_not = is_not))

        next_space = subquery.find(' ', start_index)#is the second word
        if next_space < 0:
            # No more literals in this subquery.
            length_out = sub_length - start_index
        else:
            length_out = next_space - start_index#is the total distance
        # This is a term literal containing a single term.
        return BooleanQueryParser._Literal(BooleanQueryParser._StringBounds(start_index, length_out),
        TermLiteral(subquery[start_index:start_index + length_out],is_not = is_not))
